"""
Simple MCQ evaluation pipeline for Gemini models with JSON mode
"""
...
def evaluate_question(question_data, model=""):
    """Evaluate a single MCQ question using Gemini with JSON mode"""
    try:
        client = get_client()
        start = time.time()
...
def main():
    ...
    max_workers = 4  # Gemini rate limits
    ...
    output_file = f"eval/mcq_results/{dataset_name}_gemini_{model_short}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
