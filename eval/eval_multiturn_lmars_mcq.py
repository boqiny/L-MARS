"""
MCQ evaluation script for L-MARS Multi-Turn Mode
Processes multiple-choice questions using L-MARS pipeline with iterative refinement
"""
import os
import sys
import json
...
JUDGE_MODEL = os.getenv("JUDGE_MODEL", "openai:gpt-4o")  # Judge model for multi-turn
...
MAX_ITERATIONS = 2  # Max iterations for multi-turn mode
...
class MultiTurnMCQEvaluator:
    """Evaluator for L-MARS Multi-Turn MCQ Mode."""
    
    def __init__(self, dataset_path: str = "eval/qa_f1_rules.json",
                 results_dir: str = "eval/mcq_results"):
        """
        Initialize the Multi-Turn MCQ evaluator.
    ...
    def process_single_mcq(self, item: Dict[str, Any]) -> Dict[str, Any]:
        """
        Process a single MCQ through L-MARS multi-turn pipeline.
        ...
        try:
            # Create workflow for this question with multi-turn mode
            workflow = create_workflow(
                mode="multi_turn",  # Changed to multi_turn
                llm_model=MODEL_NAME,
                judge_model=JUDGE_MODEL,  # Add judge model for multi-turn
            ...
            # Run the multi-turn workflow with just the question (no choices shown during search)
            search_result = workflow.run(question)
            
            # Get search results from the multi-turn process
            search_results = search_result.get('search_results', [])
            
            # Access the multi-turn workflow's summary agent
            multiturn_workflow = workflow.workflow
            original_summary_agent = multiturn_workflow.summary_agent
            
            # Create MCQ version of summary agent
            ...
            # Get iteration count from the result
            iteration_count = search_result.get('iteration_count', 0)
            judge_evaluations = search_result.get('judge_evaluations', [])
    ...
    def run_evaluation(self, max_questions: Optional[int] = None) -> Dict[str, Any]:
        ...
        logger.info(f"Starting Multi-Turn MCQ evaluation with {len(questions_to_eval)} questions")
        logger.info(f"Model: {MODEL_NAME}")
        logger.info(f"Judge Model: {JUDGE_MODEL}")
        logger.info(f"Mode: multi_turn_mcq")
        ...
        # Calculate average processing time and iterations
        processing_times = [r['processing_time'] for r in successful_results]
        avg_processing_time = sum(processing_times) / len(processing_times) if processing_times else 0
        
        iterations_used = [r.get('iterations', 0) for r in successful_results]
        avg_iterations = sum(iterations_used) / len(iterations_used) if iterations_used else 0
        ...
        # Create summary
        summary = {
            'run_name': f"{dataset_name}_multiturn_lmars_{MODEL_NAME.replace(':', '_')}_{datetime.now().strftime('%Y%m%d_%H%M%S')}",
            'model': MODEL_NAME,
            ...
            'iteration_distribution': self._calculate_iteration_distribution(successful_results)
        }
        ...
        output_filename = f"{dataset_name}_multiturn_lmars_{MODEL_NAME.replace(':', '_')}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        output_file = self.results_dir / output_filename
        with open(output_file, 'w') as f:
            json.dump(full_results, f, indent=2)
        
        logger.info(f"\n{'='*60}")
        logger.info(f"Multi-Turn Evaluation Complete!")
        ...
        logger.info(f"Average Iterations: {summary['average_iterations']:.2f}")
    ...
    def _calculate_iteration_distribution(self, results: List[Dict]) -> Dict[int, int]:
        """Calculate distribution of iterations used."""
        distribution = {}
        for r in results:
            iterations = r.get('iterations', 0)
            distribution[iterations] = distribution.get(iterations, 0) + 1
        return distribution
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Evaluate L-MARS Multi-Turn on MCQ dataset')
    ...
    parser.add_argument('--judge-model', type=str,
                       default='openai:gpt-4o',
                       help='Judge model to use for multi-turn evaluation')
    parser.add_argument('--max-iterations', type=int,
                       default=2,
                       help='Maximum iterations for multi-turn mode')
    ...
    # Run evaluation
    evaluator = MultiTurnMCQEvaluator(dataset_path=args.dataset)
