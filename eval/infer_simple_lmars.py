"""
Batch evaluation script for L-MARS Simple Mode
Processes legal questions one by one using L-MARS simple workflow
"""
import os
import sys
import json
...
class SimpleModeEvaluator:
    """Evaluator for L-MARS Simple Mode."""
    ...
    def process_single_question(self, item: Dict[str, Any]) -> Dict[str, Any]:
        """
        Process a single question through L-MARS Simple Mode.
        ...
        try:
            ...
            workflow = create_workflow(
                ...
                enable_tracking=False,  # Disable tracking for eval
    ...
    def run_batch_evaluation(self, max_samples: int = None) -> List[Dict[str, Any]]:
        ...
        print(f"Starting L-MARS Simple Mode evaluation of {len(dataset)} questions...")
        print(f"Using model: {MODEL_NAME}")
        print(f"Offline RAG: {ENABLE_OFFLINE_RAG}")
        print(f"CourtListener: {ENABLE_COURTLISTENER}")
        
        # Process questions sequentially (L-MARS handles its own parallelization)
        for item in tqdm(dataset, desc="Processing"):
            ...
            if result['status'] == 'success':
                tqdm.write(f"  ✓ ID {result['id']}: U-Score = {result['evaluation']['quantitative']['u_score']:.3f}")
            else:
    ...
    def save_results(self, results: List[Dict[str, Any]], run_name: str = None):
        ...
        if not run_name:
            ...
            run_name = f"simple_mode_{MODEL_NAME.replace(':', '_')}_{timestamp}"
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Batch evaluation for L-MARS Simple Mode")
    ...
    # Run evaluation
    evaluator = SimpleModeEvaluator(dataset_path=args.dataset)
