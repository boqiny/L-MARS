"""Condition 1 — Zero-Shot baseline for Bar Exam QA."""
...
from eval.config import MAX_TOKENS_ZERO_SHOT, MODEL
from eval.prompts import BAREXAM_ZERO_SHOT


def run_zero_shot_barexam(rows: List[dict], output_path: Path, workers: int = 5) -> List[dict]:
    """Run zero-shot on Bar Exam QA rows with parallel API calls.
    ...
    def _process(row: dict) -> dict:
        ...
        try:
            raw = call_gpt(prompt_text, max_tokens=MAX_TOKENS_ZERO_SHOT)
    ...
    with ThreadPoolExecutor(max_workers=workers) as pool:
        ...
        for future in tqdm(as_completed(futures), total=len(futures), desc="zero_shot/barexam"):
            try:
